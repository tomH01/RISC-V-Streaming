module buffer_pool #(
    ...
    if (!rst_ni) begin
      ...
      for (int i = 0; i < M_MACROS; i++) begin
        if (macro_owner_q[i] == 1'b0) begin
          if (ingr_done_i[i]) begin
            macro_owner_q[i] <= 1'b1;
          end
        end else begin
          if (egr_release_i[i]) begin
  ...
  always_comb begin
    for (int p = 0; p < NUM_READ_PORTS; p++) begin
      ...
      egr_r_rdata_o[p] = macro_r_rdata[egr_macro_select_q[p]];
      egr_r_valid_o[p] = macro_r_valid[egr_macro_select_q[p]];
    end
  end
    ...
    for (genvar i = 0; i < M_MACROS; i++) begin : gen_buffer_array
      always_comb begin
        ...
        if (macro_owner_q[i] == 1'b0) begin
          ...
          // Egress - READ
          for (int p = 0; p < NUM_READ_PORTS; p++) begin
            if (egr_macro_select_i[p] == i) begin
              mux_req[i]  |= egr_req_i[p];
