...
async def stream_driver(dut, expected_queue, num_items):
    dut.valid_i.value = 0
...
async def stream_receiver(dut, expected_queue, num_items):
    dut.ready_i.value = 0
    items_received = 0

    while items_received < num_items:
        dut.ready_i.value = 1 if random.random() < 0.7 else 0

        await ReadOnly()

        if dut.valid_o.value == 1 and dut.ready_i.value == 1:
            actual_data = int(dut.data_o.value)
...
async def coverage_collector(dut):
    while True:
        await RisingEdge(dut.clk_i)
        await ReadOnly()
        sample_input(dut.valid_i.value, dut.ready_o.value)
        sample_output(dut.valid_o.value, dut.ready_i.value)
...
async def test_in_stream_channel(dut):
    ...
    driver_task = cocotb.start_soon(stream_driver(dut, expected_queue, NUM_ITEMS))
    receiver_task = cocotb.start_soon(stream_receiver(dut, expected_queue, NUM_ITEMS))
